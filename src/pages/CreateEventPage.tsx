...
export default function CreateEventPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FDF1EF] to-[#F7D6D2] p-4 py-6 sm:py-8">
      <div className="max-w-4xl mx-auto">
        <button
          onClick={() => navigate('/dashboard')}
          className="mb-4 sm:mb-6 flex items-center gap-2 text-gray-700 hover:text-gray-900 transition"
        >
          <ArrowLeft size={20} />
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-4 sm:mb-6">Create Game Event</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                ...
                <button
                  type="button"
                  onClick={addPrize}
                  className="flex items-center justify-center gap-2 text-[#E56353] hover:text-[#D55445] font-medium transition bg-[#FDF1EF] hover:bg-[#FCE8E5] px-4 py-2 rounded-lg"
                >
                  <Plus size={20} />
                  <span>Add Prize</span>
                </button>
              ...
              <div className="space-y-3 sm:space-y-4">
                {prizes.map((prize, index) => (
                  <div
                    key={index}
                    className="flex flex-col sm:flex-row gap-3 items-start p-3 sm:p-4 bg-gray-50 rounded-lg border border-gray-200"
                  >
                    <div className="flex-1 w-full grid grid-cols-1 gap-3">
                      <input
                        ...
                        className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E56353] focus:border-transparent outline-none text-sm sm:text-base"
                        required
                      />
                      <div className="grid grid-cols-2 gap-3">
                        <input
                          ...
                          required
                        />
                        <label className="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-gray-300">
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="text-xs sm:text-sm text-gray-700">Joke/Blank</span>
                        </label>
                      </div>
                    </div>
                    {prizes.length > 1 && (
                      <button
                        ...
                        className="text-red-600 hover:text-red-700 transition p-2 hover:bg-red-50 rounded-lg self-center sm:self-start"
                      >
                        <Trash2 size={20} />
                      </button>
                    )}
              ...
              <p className="text-xs sm:text-sm text-gray-600 mt-3 bg-blue-50 p-3 rounded-lg">
                <strong>Requirements:</strong> Minimum 5 prizes required. At least 1 must be a real prize with value {'>'} 0.
              </p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#E56353] hover:bg-[#D55445] text-white font-semibold py-3 px-6 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
            >
